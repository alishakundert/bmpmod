...
def plt_mcmc_freeparam(mcmc_results,samples,tspec_data):
    ...
    plt.annotate('$r_{\mathrm{ref}}$='+str(int(tspec_data['radius'][params.refindex]))+' kpc',(0.7,0.9),xycoords='figure fraction')
...
def plt_summary(ne_data,tspec_data,nemodel,mcmc_results):
    ...
    plt_densityprof(nemodel)

    '''
    final kT profile with c, rs
    '''

    tfit_arr=massmod.Tmodel_func(mcmc_results['c'][0], mcmc_results['rs'][0], mcmc_results['normsersic'][0], ne_data, tspec_data, nemodel)
    ...
    plt.annotate('$r_{\mathrm{ref}}$='+str(int(tspec_data['radius'][params.refindex]))+' kpc',(0.05,0.9),xycoords='axes fraction')
    ...
    mass_nfw=massmod.nfw_mass_model(xplot,mcmc_results['c'][0],mcmc_results['rs'][0])/uconv.Msun
    mass_dev=massmod.sersic_mass_model(xplot,mcmc_results['normsersic'][0]) #Msun

    intfunc = lambda x: massmod.mgas_intmodel(x,nemodel['parvals'])
    mass_gas=[]
    for xx in xplot:
    ...
    plt.annotate(r'$c_{'+str(int(params.overdensity))+'} = '+str(np.round(mcmc_results['c'][0],decimals=1))+'_{-'+str(np.round(mcmc_results['c'][2],decimals=2))+'}^{+'+str(np.round(mcmc_results['c'][1],decimals=2))+'}$',(0.55,0.45),xycoords='figure fraction')
